...
def get_all_mask_points(masks_array):
    mask_points_all=[]
    mask_ims = []
    for mask in masks_array:
        img = np.stack((mask,) * 3, axis=-1)
...
def get_normalization(source_dir, filename, use_linear_init=False):
    ...
    dataset=dict(np.load('{0}/{1}.npz'.format(source_dir, filename)))
    masks_array = dataset['masks']
    pose_projection = dataset['camera_projection_matrix']

    mask_points_all,masks_all=get_all_mask_points(masks_array)
    number_of_cameras = len(masks_all)
    Ps = get_Ps(pose_projection)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--filename', type=str, default='', help='data source file for preprocess')
    ...
    if opt.dtu:
        ...
        get_normalization(opt.source_dir, opt.filename, opt.use_linear_init)
