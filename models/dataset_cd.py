...
class Dataset:
    def __init__(self, conf):
        ...
        self.scale_mat_scale = conf.get_float('scale_mat_scale', default=1.0)
        ...
        intrinsic_mat[0, 0] = self.camera_dict['fl_x']
        intrinsic_mat[1, 1] = self.camera_dict['fl_y']
        intrinsic_mat[0, 2] = self.camera_dict['cx']
        intrinsic_mat[1, 2] = self.camera_dict['cy']
        intrinsic_mat[2, 2] = 1
        intrinsic_mat[3, 3] = 1

        self.intrinsics_all = np.tile(intrinsic_mat, (self.n_images, 1, 1))

        # world_mat is a projection matrix from world to image
        # self.world_mats_np = [camera_dict['world_mat_%d' % idx].astype(np.float32) for idx in range(self.n_images)]
        self.world_mats_np = np.array([np.array(self.camera_dict['frames'][i]['transform_matrix']) for i in range(len(self.camera_dict['frames']))])

        self.scale_mats_np = []

        # scale_mat: used for coordinate normalization, we assume the scene to render is inside a unit sphere at origin.
        # self.scale_mats_np = [camera_dict['scale_mat_%d' % idx].astype(np.float32) for idx in range(self.n_images)]
        self.scale_mats_np = [np.eye(4) * self.scale_mat_scale for _ in range(self.n_images)]

        # self.intrinsics_all = []
        # self.pose_all = []

        self.pose_all = torch.from_numpy(self.world_mats_np.astype(np.float32)).to(self.device)  # [n_images, 4, 4]
        self.pose_all = torch.inverse(self.pose_all)  # [n_images, 4, 4]
